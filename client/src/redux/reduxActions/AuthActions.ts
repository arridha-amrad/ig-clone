...
const dispatchMessage = (dispatch: Dispatch<any>, message: string, type: messageActions.MessageTypes) => {
   dispatch(messageActions.setMessage(message, type));
};

export const uploadAvatar = (formData: FormData) => async (dispatch: Dispatch<AuthActionsType>) => {
   dispatch({ type: "LOADING_AUTH" });
   try {
...
export const updateUserData =
   (data: Partial<AuthenticatedUserData>) => async (dispatch: Dispatch<AuthMessageAction>) => {
...
export const getLoginUserData = () => async (dispatch: Dispatch<AuthActionsType>) => {
   dispatch({ type: "LOADING_AUTH" });
   try {
...
export const verifyEmail = (data: VerifyEmailData) => async (dispatch: Dispatch<AuthMessageAction>) => {
   dispatchRequiredActions(dispatch);
   try {
...
export const login = (loginData: LoginData) => async (dispatch: Dispatch<AuthMessageAction>) => {
   dispatchRequiredActions(dispatch);
   try {
      const res = (await axiosInstance.post("/auth/login", loginData)) as AxiosResponse<AuthenticatedUserData>;
      dispatch({
         type: "LOGIN_SUCCESS",
      });
      dispatch({
...
export const forgotPassword = (formData: ForgotPasswordData) => async (dispatch: Dispatch<AuthMessageAction>) => {
   dispatchRequiredActions(dispatch);
   try {
      const res = (await axiosInstance.post("/auth/forgot-password", formData)) as AxiosResponse<string>;
      dispatch({
         type: "AUTH_SUCCESS",
      });
...
export const resetPassword = (formData: ResetPasswordData) => async (dispatch: Dispatch<AuthMessageAction>) => {
   dispatchRequiredActions(dispatch);
   try {
      const res = (await axiosInstance.post(`/auth/reset-password/${formData.token}`, {
