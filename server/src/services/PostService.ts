...
export const getPost = async (postId: string): Promise<IPostModel> => {
   return PostModel.findById(postId, null, { sort: '-createdAt' })
...
export const findPosts = async (): Promise<IPostModel[]> => {
   const res = await PostModel.find({}, null, { sort: '-createdAt' })
      .populate('user', 'username imageURL')
      .populate({
         path: 'comments',
         populate: [
